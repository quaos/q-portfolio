...
export const AppContextProvider = (
  { basePath, backendApiUrl, swMessagesDispatcher, children }:
    React.PropsWithChildren<AppContextProviderProps>,
) => {
  const [loading, setLoading] = useState(true);
  // let [mobileMode, setMobileMode] = React.useState(false);
  const mobileMode = (/Android|webOS|iPhone|iPad|iPod|BlackBerry/i.test(
  ...
  return (
    <AppContext.Provider
      value={{
        ...
        basePath,
        baseUrl,
        backendApiUrl: backendApiUrl ?? baseUrl.toString(),
        mobileMode,
      }}
    >

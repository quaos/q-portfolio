...
export interface PartConfigProps<T> {
  CellComponent?: DynamicComponent;
  className?: string;
...
export interface PartProps<T> extends PartConfigProps<T>, PartRenderContext<T> {
}
...
export function Part<T>({
  ...
  switch (partType) {
    case PartType.Header:
      ...
      partProps.className = partProps.className ??
        "table-part table-header container";
      break;
    case PartType.Footer:
      isHeaderOrFooter = true;
      partProps.className = partProps.className ??
        "table-part table-footer container";
      break;
    case PartType.Body:
      partProps.className = partProps.className ??
        "table-part table-body container";
      break;
    default:
      break;
  }
  ...
  return (
    <tr {...rowProps}>
      {columns.map((column: any, colIdx: number) => {
        ...
        const sourceImpl =
          ((column.source) && (typeof column.source === "function"))
            ? column.source
...
export interface CellProps<T>
  extends CellConfigProps<T>, CellRenderContext<T> {}
...
export function Cell<T>({
  ...
  return <Component {...cellProps}>{valueStr}</Component>;
}

function useDefaultHeaderProps<T>(columns: (ColumnType<T>)[]) {
  return {
    columns,
...
function useDefaultDataCellSource<T extends Record<string, unknown>>() {
  return (
    rowData: T,
    ctx: CellRenderContext<T>,
    _config: CellConfigProps<T>,
    ...
    if (typeof ctx.column === "string") {
      return (ctx.column in rowData) ? (rowData as any)[ctx.column] : undefined;
    }

    if (typeof ctx.column.source === "string") {
      return (ctx.column.source in rowData)
        ? (rowData as any)[ctx.column.source]
        : undefined;
    }

    return (ctx.column.name in rowData)
      ? (rowData as any)[ctx.column.name]
      : undefined;
  };
}
